...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Canada eCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Canada eCoin is downloading blocks...");
...
Value getworkaux(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Canada eCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Canada eCoin is downloading blocks...");

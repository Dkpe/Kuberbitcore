...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Kuberbitcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Kuberbitcoin is downloading blocks...");
...
Value getworkaux(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Kuberbitcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Kuberbitcoin is downloading blocks...");
